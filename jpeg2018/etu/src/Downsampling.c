...
struct MCU_16 * downsampling_horizontal_2_blocs(struct MCU_16 * MCU_entree)   {
    ...
    int16_t *flux_sortie = malloc(MCU_entree->hauteur * 64 * sizeof(int16_t));
    for (int8_t bloc_vertical = 0; bloc_vertical<MCU_entree->hauteur; bloc_vertical++) {
        for (int i = 0; i < 64; i++) {
            flux_sortie[64*bloc_vertical + i] = (MCU_entree->flux[2 * (64*bloc_vertical + i)] + MCU_entree->flux[2 * (64*bloc_vertical + i) + 1]) / (int16_t) 2;
        }
    }
    struct MCU_16 *MCU_sortie = malloc(sizeof(struct MCU_16));
...
struct MCU_16 * downsampling_vertical_2_blocs(struct MCU_16 * MCU_entree)   {
    ...
    int16_t *flux_sortie = malloc(MCU_entree->largeur * 64 * sizeof(int16_t));
    for (int ligne=0; ligne < 8; ligne++)   {
        for (int i=0; i < MCU_entree->largeur * 8; i++) {
            flux_sortie[(ligne * MCU_entree->largeur * 8) + i] = (MCU_entree->flux[(MCU_entree->largeur * 8 * 2*ligne) + i]
                    + MCU_entree->flux[(MCU_entree->largeur * 8 * (2*ligne + 1)) + i]) / (int16_t) 2;
        }
    }
    struct MCU_16 *MCU_sortie = malloc(sizeof(struct MCU_16));
